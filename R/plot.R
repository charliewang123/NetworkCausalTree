...
plot_NCT <- function(NCT, 
  ...
  NCT$FILTER <- gsub(pattern = "_bin", replacement ="", 
                     x = as.character(NCT$FILTER)) 
  NCT$FILTER[NCT$FILTER != "NA"] <- paste0("NA & ", NCT$FILTER[NCT$FILTER != "NA"]) 
  
  NCT$FILTER[NCT$FILTER == "NA"] <- "NA" 
  ...
  cols <- grDevices::colorRampPalette(c(vertex_color[1], vertex_color[2]))(4) 
  
  qeff <- as.numeric(stats::quantile( V(grafo_tree)$stat
                                      [V(grafo_tree)$stat >= 0] ))[2:4] 
  
  V(grafo_tree)$color <- "white"
  
  stat_vals <- V(grafo_tree)$stat 
  pos_idx <- which(stat_vals >= 0) 
  
  V(grafo_tree)$color[pos_idx[ stat_vals[pos_idx] < qeff[1] ]] <- cols[1] 
  V(grafo_tree)$color[pos_idx[ stat_vals[pos_idx] >= qeff[1] 
                               & stat_vals[pos_idx] < qeff[2] ]] <- cols[2] 
  V(grafo_tree)$color[pos_idx[ stat_vals[pos_idx] >= qeff[2] 
                               & stat_vals[pos_idx] < qeff[3] ]] <- cols[3] 
  V(grafo_tree)$color[pos_idx[ stat_vals[pos_idx] >= qeff[3] ]] <- cols[4] 
  ...
  NCTPLOT <- plot(grafo_tree, layout = layout_as_tree(grafo_tree), 
